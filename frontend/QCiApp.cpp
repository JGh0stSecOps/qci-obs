...
static bool MakeUserDirs()
{
	...
	if (GetAppConfigPath(path, sizeof(path), OBS_USER_DATA_DIR "/basic") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
	...
	if (GetAppConfigPath(path, sizeof(path), OBS_USER_DATA_DIR "/logs") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
	...
	if (GetAppConfigPath(path, sizeof(path), OBS_USER_DATA_DIR "/profiler_data") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
...
#ifdef _WIN32
	if (GetAppConfigPath(path, sizeof(path), OBS_USER_DATA_DIR "/crashes") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
	...
	if (GetAppConfigPath(path, sizeof(path), OBS_USER_DATA_DIR "/updates") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
	...
	if (GetAppConfigPath(path, sizeof(path), OBS_USER_DATA_DIR "/plugin_config") <= 0) {
		return false;
	}
	if (!do_mkdir(path)) {
		return false;
...
constexpr std::string_view OBSProfileSubDirectory = OBS_USER_DATA_DIR "/basic/profiles";
constexpr std::string_view OBSScenesSubDirectory = OBS_USER_DATA_DIR "/basic/scenes";
constexpr std::string_view OBSPluginManagerSubDirectory = OBS_USER_DATA_DIR "/plugin_manager";
...
bool OBSApp::InitGlobalConfig()
{
	...
	int len = GetAppConfigPath(path, sizeof(path), OBS_USER_DATA_DIR "/global.ini");
	if (len <= 0) {
		return false;
	}
...
{
	const std::string userConfigFile = userConfigLocation.u8string() + "/" OBS_USER_DATA_DIR "/user.ini";
...
static constexpr string_view OBSGlobalIniPath = "/" OBS_USER_DATA_DIR "/global.ini";
static constexpr string_view OBSUserIniPath = "/" OBS_USER_DATA_DIR "/user.ini";
	  ...
	  appLaunchUUID_(QUuid::createUuid())
{
	...
	sleepInhibitor = os_inhibit_sleep_create(OBS_PRODUCT_NAME " Video/audio");
...
static void move_basic_to_profiles(void)
{
	...
	if (GetAppConfigPath(path, 512, OBS_USER_DATA_DIR "/basic") <= 0) {
		return;
	}
	...
	const std::filesystem::path profilesPath =
		App()->userProfilesLocation / std::filesystem::u8path(OBS_USER_DATA_DIR "/basic/profiles");
...
static void move_basic_to_scene_collections(void)
{
	...
	const std::filesystem::path sceneCollectionPath =
		App()->userScenesLocation / std::filesystem::u8path(OBS_USER_DATA_DIR "/basic/scenes");
...
bool OBSApp::OBSInit()
{
	...
	blog(LOG_INFO, "Hide " OBS_PRODUCT_NAME " windows from screen capture: %s", hideFromCapture ? "true" : "false");
#endif
...
void OBSApp::uploadLastAppLog() const
{
	...
	basicWindow->UploadLog(OBS_USER_DATA_DIR "/logs", GetLastLog(), OBS::LogFileType::LastAppLog);
}
...
void OBSApp::uploadCurrentAppLog() const
{
	...
	basicWindow->UploadLog(OBS_USER_DATA_DIR "/logs", GetCurrentLog(), OBS::LogFileType::CurrentAppLog);
}
