...
static void ClearShaderCache()
{
	...
	if (SHGetFolderPathW(NULL, CSIDL_COMMON_APPDATA, NULL, SHGFP_TYPE_CURRENT, shader_path) == S_OK) {
		if (SUCCEEDED(StringCbCatW(shader_path, sizeof(shader_path), L"\\" OBS_USER_DATA_DIR_W L"\\shader-cache"))) {
...
static bool Update(wchar_t *cmdLine)
{
	...
	StringCbCat(lpAppDataPath, sizeof(lpAppDataPath), L"\\" OBS_USER_DATA_DIR_W);
	...
	if (!GetTempFileNameW(tempDirName, OBS_USER_DATA_DIR_W, 0, tempPath)) {
