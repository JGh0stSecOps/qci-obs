...
#include <utility/QCiTranslator.hpp>
...
static void create_log_file(fstream &logFile)
{
	...
	get_last_log(false, OBS_USER_DATA_DIR "/logs", lastLogFile);

	currentLogFile = GenerateTimeDateFilename("txt");
	dst << OBS_USER_DATA_DIR "/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file(false, OBS_USER_DATA_DIR "/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE(OBS_USER_DATA_DIR "/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	if (!isInBundle()) {
		blog(LOG_ERROR, OBS_PRODUCT_NAME
		     " cannot be run as a standalone binary on macOS. Run the Application bundle instead.");
		return ret;
	}
#endif
	...
	try {
		...
		program.AppInit();
		delete_oldest_file(false, OBS_USER_DATA_DIR "/profiler_data");
		...
		if (multi) {
			blog(LOG_INFO, "User enabled --multi flag and is now "
				       "running multiple instances of " OBS_PRODUCT_NAME ".");
		} else {
			blog(LOG_WARNING, "================================");
			blog(LOG_WARNING, "Warning: " OBS_PRODUCT_NAME " is already running!");
			blog(LOG_WARNING, "================================");
			blog(LOG_WARNING, "User is now running multiple "
					  "instances of " OBS_PRODUCT_NAME "!");
...
#define CRASH_MESSAGE                                                                    \
	"Woops, " OBS_PRODUCT_NAME " has crashed!\n\nWould you like to copy the crash log " \
...
static void main_crash_handler(const char *format, va_list args, void * /* param */)
{
	...
	string crashFilePath = OBS_USER_DATA_DIR "/crashes";
	...
	int ret = MessageBoxA(NULL, finalMessage.c_str(), OBS_PRODUCT_NAME " has crashed!",
...
static constexpr char vcRunErrorMsg[] = OBS_PRODUCT_NAME
	" requires a newer version of the Microsoft Visual C++ "
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		if (arg_is(argv[i], "--multi", "-m")) {
			...
			std::cout << OBS_PRODUCT_NAME " - " << App()->GetVersionString(false) << "\n";
			exit(0);
		}
	}
