...
OBSBasic::OBSBasic(QWidget *parent) : OBSMainWindow(parent), undo_s(ui), ui(new Ui::OBSBasic)
{
	...
	stringstream name;
	name << OBS_PRODUCT_NAME " " << App()->GetVersionString();
...
void OBSBasic::UpdateTitleBar()
{
	...
	name << OBS_PRODUCT_NAME " " << App()->GetVersionString(false);

	/* Upstream titled itself "OBS" normally and "OBS Studio" only in preview/program mode — a pun
	 * on its own product name that does not survive the rename, since the product name already
	 * ends in "Studio" ("QCi Studio Studio"). Studio mode now gets the same parenthetical the two
	 * modes below already use, reusing the existing menu label so no new locale key is needed. */
