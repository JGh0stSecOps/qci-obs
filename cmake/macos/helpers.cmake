    ...
    if(target STREQUAL obs-studio)
      ...
      if(
        ...
        message(
          ...
          "\n"
          "    THE RECIPE THAT WORKS, measured on this Mac 2026-08-12 — the extension went from "
          "absent to embedded and correctly signed:\n"
          "\n"
          "        cmake build_macos -DOBS_CODESIGN_IDENTITY= -DOBS_CODESIGN_TEAM=${OBS_CODESIGN_TEAM}\n"
          "        cmake --build build_macos --config RelWithDebInfo -- -allowProvisioningUpdates\n"
          "\n"
          "    Why each half is needed, because neither is guessable from the failure alone:\n"
          "      * CLEARING the identity is what selects automatic signing. cmake/macos/xcode.cmake "
          "picks Manual whenever OBS_CODESIGN_IDENTITY is a real (non-adhoc) certificate, so passing "
          "a Developer ID here and asking for Automatic are mutually exclusive by construction. "
          "Setting CMAKE_XCODE_ATTRIBUTE_CODE_SIGN_STYLE directly does NOT work — xcode.cmake "
          "overwrites it.\n"
          "      * -allowProvisioningUpdates is what lets Xcode REGISTER the App IDs and mint the "
          "profile. Without it the build fails with \"No profiles for "
          "'solutions.zoetic.qci-studio' were found\" even while signed in, because no profile for "
          "these bundle ids has ever existed.\n"
          "\n"
          "    THE COST, so it is a decision and not a surprise: this signs with Apple Development, "
          "not Developer ID, so 'spctl -a' REJECTS the bundle. It launches locally (a build tree "
          "carries no quarantine attribute) but it is not distributable, not notarizable, and "
          "changing back to Developer ID later changes the app's designated requirement — which "
          "invalidates every TCC grant (screen recording, camera, microphone) the bundle id has "
          "accumulated. Developer ID + system extension needs a Developer ID provisioning profile "
          "carrying the entitlement, created in the developer portal; Xcode's automatic signing "
          "mints Development profiles only.\n"
          "\n"
          "    Or set PROVISIONING_PROFILE in the environment before configuring, or accept that "
          "this build has no virtual camera."
        )
      endif()
